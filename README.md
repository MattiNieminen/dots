...
Then enable HDR from the same view.
...
Middle click paste may not be enabled by default (blame Gnome, not Bazzite). To
...
If this prints `false` and you want to enable middle click paste, run the
following command:
...
environment. The containers are setup using an idempotent wrapper script:
...
Because the script is idempotent, Distrobox apps can be upgraded by running the
script again. Just make sure you close all the exported apps and binaries
before doing it.
...
startup. Remember to restore your bookmarks and make bookmarks bar visible
only on new pages. Make Google your default search engine, as the default
search from Brave is pretty awful.

Upgrading Brave during startup may occasionally fail. This can be fixed by
upgrading the Distrobox containers by running the wrapper script again. Just
make sure you close all the apps and binaries exported from the Distrobox
containers.

#### Development environment

The development environment is installed using Distrobox similar to Brave. So
if you followed the Distrobox section of this README.md, you already have
development related GUI apps and CLI tools at our disposal.

Upgrading the apps can be done by running the wrapper script again. At the risk
of repeating myself too much: remember to close all apps and binaries exported
from Distrobox containers.
